...
public class SmallPuncherScreen extends AbstractSimiScreen {
    ...
    public SmallPuncherScreen(PlayerEntity player, boolean offhand) {
        this.renderedItem = AllItems.PUNCHER.asStack();
        this.background = AllGuiTextures.SMALL_PUNCH_CARD_PROGRAMMER;
    ...
    protected void init() {
        ...
        if(stack.getItem() instanceof PunchCardItem && stack.getOrCreateTag().contains(CardData.PROGRAM_TAG)) {
            CardData program = CardData.deserialize(stack.getOrCreateTag(), true);
            if(program != null) {
                for(int x = 0; x < 2; x++) {
