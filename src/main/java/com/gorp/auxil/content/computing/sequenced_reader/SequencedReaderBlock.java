...
public class SequencedReaderBlock extends HorizontalKineticBlock implements ITE<SequencedReaderTileEntity> {
    ...
    public SequencedReaderBlock(Properties properties) {
        super(properties);
    ...
    public BlockState getStateForPlacement(BlockItemUseContext context) {
        ...
        return state.setValue(HORIZONTAL_FACING, state.getValue(HORIZONTAL_FACING).getOpposite()).setValue(ON, false);
    }
    ...
    public int getSignal(BlockState blockState, IBlockReader blockAccess, BlockPos pos, Direction side) {
        ...
        if(tile instanceof SequencedReaderTileEntity && blockState.getValue(HORIZONTAL_FACING).getAxis().test(side)) {
            return ((SequencedReaderTileEntity)tile).getPower();
        }
        return 0;
    }
    ...
    private void updatePower(World pWorld, BlockPos pPos, BlockState pState) {
        ...
        final int finalPower = pWorld.getSignal(otherPowerPosition, pState.getValue(HORIZONTAL_FACING).getOpposite());
        ...
        withTileEntityDo(pWorld, pPos, (tileEntity) -> {
                ...
                tileEntity.tryStep(finalPower);
        });

        if(finalPower > 0 != pState.getValue(ON))
            pWorld.setBlock(pPos, pState.setValue(ON, finalPower > 0), 51);
    }
        ...
        if(inHand.getItem() instanceof PunchCardItem) {
            ...
            if(items != null && !items.getStackInSlot(0).isEmpty()) {
                if(player.addItem(items.getStackInSlot(0))) {
                    ...
                    BlockPos possToAdd = pos.relative(state.getValue(SequencedReaderBlock.HORIZONTAL_FACING));
    ...
    public TileEntity createTileEntity(BlockState blockState, IBlockReader iBlockReader) {
        return AllTileEntities.SEQUENCED_READER.create();
    }
    ...
    @Override
    public Class<SequencedReaderTileEntity> getTileEntityClass() {
        return SequencedReaderTileEntity.class;
    }
